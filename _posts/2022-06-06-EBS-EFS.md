...
```
10.0.0.233:/ 		/data 	nfs4    nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,noresvport
# 형태: <NFS MOUNT IP>:/ 		/data 	nfs4 	 <OPTIONS> 0 0
# NFS MOUNT IP 및 OPTIONS 는 EFS - Attach 에서 확인했던 값을 사용하면 된다.
# 중간에 tab 두번, 한번, 한번이 각각 들어있다.
```
다른 EC2 에서도 /etc/fstab 에 해당 라인을 추가하면 EFS 가 부팅 시 자동으로 mount 되게 된다.
